  ...
  const generateAIInsights = async (entryId: string, entryContent: string, entryMood: string) => {
    ...
    try {
      const response = await fetch('/api/ai', {
        ...
        body: JSON.stringify({
          prompt: `Based on this journal entry, provide 3-4 brief suggestions for tomorrow in this format:
...
Mood: ${entryMood}`
      ...
      setAiInsights(prev => ({ ...prev, [entryId]: data.response }));
      setShowAIInsights(entryId);
    } catch (error) {
      console.error('Error generating insights:', error);
      setAiInsights(prev => ({ ...prev, [entryId]: 'Unable to generate insights at the moment.' }));
      setShowAIInsights(entryId);
    } finally {
      setLoadingAI(null);
    }
